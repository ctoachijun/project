...
function load_times(){
  ...
  for ($i=0; $i < 15 ; $i++) {
    $nows = date('Y-m-d');
		$date = strtotime("{$nows} +{$i} days");
		$arr['text'][] = date('Y년m월d일',$date);
    $arr['week'][] = $week[date('w',$date)];
    $arr['val'][] = date('Y-m-d',$date);
	}
  return $arr;
}

function ro_load_times($rc_start){
  $week = ['일','월','화','수','목','금','토'];
  $rc_start_date = strtotime($rc_start);
  $st = 1 - date('w',$rc_start_date);
  $la = date('w',$rc_start_date) - date('w',$rc_start_date);
  $arr = [];
  $str_now = strtotime(date("Y-m-d"));
  $str_target = strtotime($rc_start);

  if($str_target > $str_now){
    for ($i= $st ; $i < 5; $i++) {
      $nows = $rc_start;
      $date = strtotime("{$nows} {$i} days");
      $arr['text'][] = date('Y-m-d',$date);
      $arr['val'][] = date('Y-m-d',$date);
      $arr['week'][] = $week[date('w',$date)];
      if(date('w',$date) == 5){break;}
    }
  }
  return $arr;
}
...
function sw_wa_con_icon($wa_con){
  switch ($wa_con) {
    case '000':
      $rs = "fa fa-pause";
    break;
    case '001':
      $rs = "fa fa-assistive-listening-systems";
    break;
    case '002':
      $rs = "fa fa-check";
    break;
...
function sw_ro_con_icon($wa_con){
  switch ($wa_con) {
    case '000':
      $rs = "fa fa-pause";
    break;
...
function sw_rc_con_icon($wa_con){
  switch ($wa_con) {
    case '000':
      $rs = "fa fa-pause";
    break;
...
function load_latlng($address){
  ...
  $locations = kakao_location_call($keys,$params);
  return $locations;
}


///-------------------------------------------------------------------------------------------------------------//
function kakao_location_sido($x,$y){
   ...
   $get_url = "https://dapi.kakao.com/v2/local/geo/coord2regioncode.json?x={$x}&y={$y}";
...
// 주소를 넣으면 좌표로 변환함
function load_sido($address){
  $address_url = urlencode($address);

  $params = "query=".$address_url;
  $locations = kakao_location_call($params);
  return $locations;
}

function send_al($tpl_code,$receiver,$recvname,$subject,$message,$b_name,$b_link){

  $token = "b1ed772212295cacb2a7adeb65f3e3069962cceb9fd6fde5a9783ae2af3e489e1ffd82f4c27526b4afe8c1d18801b232322661144e4734ce0b039859428f8d91BRMqRerH6YghTd/zS4iCWnLX73X8PsNwg9kCO1kVmPvOTD6s9iSjxZcTkHVhSABzgiZpLJqFJis81xLP6W+/cg==";
  $_apiURL    =	'https://kakaoapi.aligo.in/akv10/alimtalk/send/';
  $_hostInfo  =	parse_url($_apiURL);
  $_port      =	(strtolower($_hostInfo['scheme']) == 'https') ? 443 : 80;
  $_variables =	array(
    'apikey'      => '2y7vhh09nfe9qyw6pse1qcho0y8iyxo3',
    'userid'      => 'sechain',
    'token'       => $token,
    'senderkey'   => '9e8938b1ac33489b6932196eb1824d4a47410113',
    'tpl_code'    => $tpl_code,
    'sender'      => '1544-5969',
    'receiver_1'  => $receiver,
    'recvname_1'  => $recvname,
  ...
  // 버튼이 있을떄 넣어줌
  if($b_name != "" && $b_link != "" ){
      $_variables['button_1'] = '{"button":[{"name":"'.$b_name.'","linkType":"WL","linkP":"'.$b_link.'", "linkM": "'.$b_link.'"}]}';
  }
  /*
  ...
  */
  ...
  // JSON 문자열 배열 변환
  $retArr = json_decode($ret,JSON_UNESCAPED_UNICODE);
  return $retArr;
  /*
...
function send_push($keys,$title,$content,$type=""){
  //type 값 (mathching,pay)
  //서버키 변경하시면 안되요
  if($type == 1){
    $API_ACCESS_KEY='AAAAb6Ela7g:APA91bHiMokAFYdI2_phySapTeUJJJGxZ1y5BRE99MsuWqMK-zol4TACZO6A41QwCUAQhtrZA-FcaOmT1sgoZdLRCG-LhGgJk9BGcQ2RD9RY4-DTM2tcuJx6awuy6mtEDFO2lxfeWw7o';// 사용자
  }
  if($type == 2){
    $API_ACCESS_KEY = "AAAAsb5le_E:APA91bGid994GyEVeCjuPSk2aevK1N8TbyHVVTLaV9vpFjPrCzUMiKXQZgWHeFNp5z_qmYm98N6lIEUERMIlF3Df9pigrqd07nedd_8ee6TnF1rxVBviS7_cvFc1FDTUH6ZVaOIXeqmt"; // 매니저
  }
...
function get_member($mb_id, $fields='*')
{
    ...
    // echo " select $fields from {$g5['member_table']} where mb_id = '{$mb_id}'";
    return sql_fetch(" select $fields from {$g5['member_table']} where mb_id = '{$mb_id}'");
